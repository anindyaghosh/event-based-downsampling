...
class eventDownsample:
    """Performs high fidelity event based spatio-temporal down-sampling.
    Arguments:
        ...
        noise_threshold (int): threshold value for number of events of each polarity in each mega-pixel allowed
                                 before a spike is emitted in the down-sampled pixel.
        timesteps_per_frame (int)
        version (string): 'naive' for naive event down-sampling -> tonic.transforms.Downsample.
                          'integ' for differentiator based event down-sampling.
                          'diff' for integrator based event down-sampling.
                          'global' -> naive event-based down-sampling with spatial_factor=1.0 and noise_threshold = 0.
    Example:
        >>> from tonic.transforms import eventDownsample
        >>> transform1 = eventDownsample(time_factor=0.001) # change us to ms
        >>> transform2 = eventDownsample(target_size=(20, 20)), version='v2', 
                                         noise_threshold=10) # reduce x/y plane to 20x20 pixels, 
                                         'v2' down-sampling and noise_threshold set to 10 events of each polarity.
    """

    sensor_size: Tuple[int, int, int] = (640, 480, 2)
    target_size: Tuple[int, int] = (20, 20)
    timesteps_per_frame: int = 33
    _noise_thr: int = 0
    
    def __init__(self, events, sensor_size, target_size, timesteps_per_frame, version, _noise_thr=0):
        self.filename = events
        self.original_resolution = sensor_size[:-1]
        self.new_resolution = target_size
    ...
    def path(self):
        ...
        # XXX: Fine for now but need to change for tonic
    ...
    def adaptive_threshold(self):
        # noise_thr[t] = noise_thr[t-1] * np.exp(-1 / THRESHOLD_TAU)
        # TODO: Implement adaptive threshold -- Tau ~ 10 ms
        # Without is alright for simple stimulus
        noise_thr = None
    ...
    def read_input_spikes(self):
        ...
        with open(self.Path, "r") as input_spike_file:
            ...
            for line in input_spike_file:
                ...
                frame_input_spikes = np.abs(frame_input_spikes.astype(np.int32))
