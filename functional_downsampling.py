...
def feedback_inhibition(events: np.ndarray, target_size: tuple, dt: float, buffer_layer_parameters: dict, inhibitory_layer_parameters: dict):
    ...
    * Activity updates
    ...
    for t, time in enumerate(np.arange(0, event_times[-1] + dt, dt)):
        ...
        buffer_to_inhibitory_synapse = (buffer_layer[t,:,:] * g_exc_B).clip(min=0)
        ...
        inhibitory_to_buffer_synapse = (inhibitory_layer[t+1,:,:] * g_exc_I).clip(min=0)
...
# Time bin of 1 ms
events = time_bin_numpy(events, 0.5)
t1 = time.perf_counter()
events_diff = diff_downsample(events, (128, 128, 2), (20, 15), noise_threshold=20)
t2 = time.perf_counter()
print(f'{t2-t1:.2f}')
events_inhib = feedback_inhibition(events_diff, (20, 15), dt=0.5, buffer_layer_parameters={"p": 0.4, "theta": 0.3, "alpha": 0.5}, inhibitory_layer_parameters={"p": 0.8})
